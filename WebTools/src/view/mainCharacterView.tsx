      ...
      <>
        ...
        <div className="row row-cols-1 row-cols-md-3 mt-3">
          <StatView
            name={t(
              character.version > 1
                ? 'Construct.other.protection'
                : 'Construct.other.resistance',
            )}
          ...
          />
          <StatView
            name={t('Construct.other.reputation')}
          ...
          />
          <StatView
            name={t('Construct.other.reprimands')}
  ...
  function renderTopFields() {
    return (
      <>
        ...
        <div className="row" style={{ alignItems: 'baseline' }}>
          <div className="col-md-2 view-field-label pb-2">
            {t('Construct.other.assignment')}:
          </div>
          <div className="col-md-10 text-white">
            <div className="view-border-bottom pb-2">{renderAssignment()}</div>
          </div>
        </div>
    ...
    if (character.isKlingonImperialCitizen) {
      return (
        <div className="row" style={{ alignItems: 'baseline' }}>
          ...
          <div className="col-md-4 text-white">
            <div className="view-border-bottom pb-2">{character.lineage}</div>
          </div>

          <div className="col-md-2 view-field-label pb-2">House:</div>
          <div className="col-md-4 text-white">
            <div className="view-border-bottom pb-2">{character.house}</div>
          </div>
        </div>
      );
    } else {
          ...
          {character.logEntries?.map((l, i) => (
            <div className="col-12 col-md-6 my-3" key={'entry-' + i}>
              <div className="pb-3 view-border-bottom h-100">
                ...
                {l.notes?.length ? (
                  <Markdown className="markdown-sm mt-2">
                    {'**' + t('Common.text.notes') + ':** ' + l.notes}
                  </Markdown>
                ) : undefined}
                {l.valuesUsed?.length ? (
                  <ul className="mt-2">
                    {l.valuesUsed?.map((v, i) => (
                      <li key={'entry-value-' + i} className="small">
                        {v.prefix + v.value}
                      </li>
                    ))}
                  </ul>
                ...
                {l.directivesUsed?.length ? (
                  <ul className="mt-2">
                    {l.directivesUsed?.map((d, i) => (
                      <li key={'entry-directive-' + i} className="small">
                        {d}
                      </li>
                    ))}
                  </ul>
