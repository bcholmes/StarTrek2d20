...
class Upbringings {
    ...
    getCaste(caste: EarlyOutlook) {
        var list = this.getEarlyOutlooksList(CharacterType.KlingonWarrior, false).filter(o => o.id === caste);
    ...
    private toList(list: EarlyOutlookModel[]) {
    ...
    getUpbringingByTypeName(typeName: string, type: CharacterType) {
        let mapping = {
            ...
            ScienceAndTechnology: EarlyOutlook[EarlyOutlook.ScientificCaste],
            ArtisticAndCreative: EarlyOutlook[EarlyOutlook.ArtisticCaste],
        ...
        let list = this.getEarlyOutlooksList(type, false);
        let filteredList = list.filter(u => EarlyOutlook[u.id] === typeName);
        if (filteredList.length === 0) {
            list = this.getEarlyOutlooksList(type, true);
            filteredList = list.filter(u => EarlyOutlook[u.id] === typeName);
        }
