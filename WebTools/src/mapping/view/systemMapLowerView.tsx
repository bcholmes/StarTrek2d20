...
class SystemMapLowerView extends React.Component<ISystemMapLowerViewProperties, {}> {
    ...
    render() {
        ...
        if (system.companionStar) {
            if (system.companionType === CompanionType.Close) {
                ...
                star2 = (<g>
                    <circle cx={35 + r} cy="110px" r={r2} fill={system.companionStar.spectralClass.colour.asHex()} stroke="black" strokeWidth={0.5} />
                    <text x={35 + r} y={110 + r + 20} fontSize={16} fontFamily="lcars" fill="white" textAnchor="middle">{system.companionStar?.designation?.toLocaleUpperCase()}</text>
                    <text x={35 + r} y={110 + r + 36} fontSize={16} fontFamily="lcars" fill="white" textAnchor="middle">{system.companionStar?.spectralClass?.colourDescription?.toLocaleUpperCase()}</text>
                </g>)
            }
        }
        ...
        let worlds = system.worlds?.map((w, i) => {
            ...
            if (w.worldClass.id === WorldClass.AsteroidBelt) {
                ...
                return (<g key={'world-' + i}>
                        <circle cx={orbitalX} cy="110px" r={r} fill={purple} />
                        {axis}
        ...
        return (<div className="starSystemLowerView">
                <svg
                    ...
                    <circle cx="35px" cy="110px" r={r} fill={system.star.spectralClass.colour.asHex()} />
    ...
    renderTrackers(orbits: number[], offset: number) {
        if (orbits?.length) {
            ...
            let lastAuMarker = this.calculateOrbitX((system.worlds[system.worlds.length - 1]).orbitalRadius, orbits);

            return (<g>
                <rect x={15} y={200} height={30} width={(inner - 2 - 15).toFixed(2)} fill="#F9AC76" />
                <rect x={(inner + 2).toFixed(2)} y={200} height={30} width={((outer-2) - (inner+2)).toFixed(2)} fill="#f7bbdd" />
                ...
                {ecoWidth >= 54 ?
                    (<text x={(inner + 6 + (ecoWidth / 2)).toFixed(2)} y={221} fontSize={16} fontFamily="lcars" fill="black" textAnchor="middle">ECOSPHERE</text>) : null}
                <text x={(outer + 6)} y={221} fontSize={16} fontFamily="lcars" fill="black" textAnchor="start">OUTER ZONE</text>

                <rect x={(lastAuMarker + 2).toFixed(2)} y={0} height={20} width={(1110 - (lastAuMarker+2)).toFixed(2)} fill="#F9AC76" />
            </g>);
        } else {
            return (<g>
