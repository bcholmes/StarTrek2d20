...
export class BasicGeneratedPortraitCharacterSheet extends BaseNonForm2eSheet {
  ...
  getName(): string {
    return i18next.t(
      makeKey('Sheet.', 'BasicGeneratedPortraitCharacterSheet'),
      { defaultValue: 'New 2nd Ed.-style Portrait Character Sheet' },
    );
  }
  getThumbnailUrl(): string {
    return '/static/img/sheets/STA_2e_Portrait_Character_Sheet.png';
  }
  ...
  getTags(): SheetTag[] {
    return [SheetTag.Portrait, SheetTag.Style2e, SheetTag.LanguageSupport];
  }
  ...
  async populate(pdf: PDFDocument, construct: Construct) {
    ...
    remainingColumn = this.writeCharacterDescription(
      page,
      character,
      remainingColumn,
      (page, column, version) => {
  ...
  writeStatBoxes(page: PDFPage, column: Column, character: Character) {
    ...
    labelWriter(
      ...
      this.boldFont,
      9,
      (label) => labelColourProvider(character.era, label),
      TextAlign.Right,
      '',
